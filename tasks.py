import aiohttp
import asyncio
#from utils_astro import calculate_natalchart, calculate_synastry, calculate_transit
...
def generate_natalchart(chat_id, input_data):
    """
    Задача для расчета натальной карты.
    """
    try:
        user = get_user_by_chat_id(chat_id)
        #subscription_status = user.subscription_active
        natalchart_svg, natal_data, tracts_planets, tracts_houses, theme = calculate_natalchart(input_data) #, subscription_status
        # print(tracts_planets)
        # print(tracts_houses)
        ...
        report = format_pretty_report(natal_data)  # Форматируем в словарь
        ...
        # Если у пользователя есть подписка, добавляем информацию о лунной фазе
        # if subscription_status:
        #     summary_text += (
        #         f"\n🌙 <b>Лунная фаза:</b> {report['lunar_phase']['emoji']} "
        #         f"{lunar_phase_translate.get(report['lunar_phase']['name'], report['lunar_phase']['name'])}"
        #     )
        ...
        asyncio.run(send_messages_to_user_async(chat_id, summary_text, reply_markup)) # вот здесь основная информация, а в кнопках выше отдельные разделы
...
def generate_transit(chat_id, input_data_person, input_data_transit):
    """
    Задача для расчета карты транзита.
    """
    try:
        user = get_user_by_chat_id(chat_id)
        #subscription_status = user.subscription_active
        # Генерация SVG и PNG
        transit_svg, aspects_svg, tractations_message, theme = calculate_transit(input_data_person, input_data_transit) #, subscription_status
        ...
        #     asyncio.run(send_messages_to_user_async(chat_id, "Для получения полных трактовок оформите подписку в профиле."))
    except Exception as e:
        # Обработка ошибок: отправка сообщения об ошибке
...
def generate_synastry(chat_id, input_data_first, input_data_second):
    """
    Задача для расчета карты синастрии.
    """
    try:
        user = get_user_by_chat_id(chat_id)
        #subscription_status = user.subscription_active
        # Генерация SVG и PNG
        synastry_svg, aspects_svg, tractations_message, theme = calculate_synastry(input_data_first, input_data_second) #, subscription_status
...
async def send_photo_to_user_async(chat_id, photo_bytes):
    """
    Асинхронная отправка фото через Telegram Bot API.
    """
    url = f"https://api.telegram.org/bot{TOKEN_BOT}/sendPhoto"
    # Формируем данные для отправки
    form_data = aiohttp.FormData()
    form_data.add_field("chat_id", str(chat_id))  # Убедимся, что chat_id — строка
...
async def send_messages_to_user_async(chat_id, message, reply_markup=None):
    """
    Асинхронная отправка текста через Telegram Bot API с возможностью добавить inline-кнопки.
    """
    ...
    if reply_markup:
        data["reply_markup"] = json.dumps(reply_markup)  # Важно, Telegram API требует JSON-формат
...
# @retry(stop=stop_after_attempt(5), wait=wait_exponential(multiplier=1, min=2, max=10))
# async def send_messages_to_user_async(chat_id, messages):
#     """
#     Асинхронная отправка текста через Telegram Bot API.
#     """
#     url = f"https://api.telegram.org/bot{TOKEN_BOT}/sendMessage"
...
async def send_error_message(chat_id, error_message):
    """
    Асинхронная отправка текстового сообщения через Telegram Bot API.
    """
    url = f"https://api.telegram.org/bot{TOKEN_BOT}/sendMessage"
