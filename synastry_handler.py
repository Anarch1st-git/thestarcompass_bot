...
# Запрос для ввода данных для синастрии
async def start_input_synastry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    #     if not can_generate(user, "synastry"):
    #         next_available_time = get_next_available_time(user, "synastry")  # Получаем время сброса
    #         next_time_str = next_available_time.strftime("%d-%m-%Y %H:%M")  # Форматируем

    #         await query.message.reply_text(
    #             text=(
    #                 "🚫 <b>Лимит исчерпан</b> 🚫\n\n"
    #                 "Вы исчерпали дневной лимит расчёта синастрии. 😔\n\n"
    ...
    #             parse_mode='HTML'
    #         )
    #         save_log_event(chat_id, "user tried generating synastry chart but limit reached")
    #         return ConversationHandler.END
...
# обработка кнопки создать карту синастрии для синастрии
...
# Обработка нажатия на кнопку "Отменить" для синастрии
...
conv_handler_synastry = ConversationHandler(
    ...
    states={
        ...
        SYNASTRY_INPUT_CITY_FIRST: [
            ...
            CallbackQueryHandler(select_city_synastry_first, pattern="^s_c_s_1\|.*$"),
            CallbackQueryHandler(cancel_create_synastry, pattern="^cancel_create_synastry$")
        ],
        SYNASTRY_SELECT_YEAR_SECOND: [
        ...
        SYNASTRY_INPUT_CITY_SECOND: [
            ...
            CallbackQueryHandler(select_city_synastry_second, pattern="^s_c_s_2\|.*$"),
            CallbackQueryHandler(cancel_create_synastry, pattern="^cancel_create_synastry$")
        ],
        SYNASTRY_CONFIRM_DATA: [
